...
.. code-block:: bash
   ...
   t.rast.extract input=sen_tile_5606 where="name like '%B8A%'" output=b8
...
.. tip:: GRASS 8 adds support for band references (see
   ...
   using :map:`sen_tile_5606` space-time dataset. No need for creating
...
.. figure:: ../images/units/07/ndvi_1.png
   ...
   Filtering produced NDVI products in :item:`Data` tab.
...
.. note:: Load data into animation tool as **multiple raster maps**
          instead of a space-time dataset. Otherwise the tool will freeze due
          to sampling. The problem is related to
          `i.sentinel.import workaround
          <https://github.com/OSGeo/grass-addons/pull/607>`__ for
          GRASS `issue 1843
          <https://github.com/OSGeo/grass/issues/1843>`__.
...
*MASK*. To disable a mask, it is enough to rename the *MASK* map by
...
.. code-block:: bash
   ...
   sentinel-cloud-mask.py map=tile_5606_1km output=cloud-timestamps.txt
...
And apply modified expression for map algebra and set *ndvi* color
table.

.. code-block:: bash

   t.rast.mapcalc input=b4,b8,clouds output=ndvi_masked \
   expression="ndvi_masked = if(isnull(clouds), null(), float(b8 - b4) / ( b8 + b4 ))" \
   basename=ndvi_masked nproc=3
...
.. figure:: ../images/units/07/simple-animation-clouds.gif
   ...
   Simple NDVI animation with clouds mask applied. Computation is limited
   to AOI only.
