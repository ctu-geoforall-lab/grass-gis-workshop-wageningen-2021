...
.. note:: Load data into animation tool as **multiple raster maps**
          instead of space time dataset. Otherwise the tool freeze due
          to sampling dataset. The problem is is related to
...
Let's check maps registered in new space-time dataset.
...
.. figure:: ../images/units/07/simple-animation-clouds.gif
   ...
   Simple NDVI animation with clouds mask applied. Computation limited
   to AOI only.
